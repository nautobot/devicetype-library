    ...
    if not device.isDevice:
        ...
        if len(matches) > 1:
            device.failureMessage = (
                f'{device.file_path} appears to be duplicated. Found {len(matches)} matches: {", ".join(matches)}'
            )
            return False
...
def validate_components(component_types, device_or_module):
    for component_type in component_types:
        ...
        if not isinstance(defined_components, list):
            device_or_module.failureMessage = (
        ...
        for idx, component in enumerate(defined_components):
            if not isinstance(component, dict):
                device_or_module.failureMessage = (
                    f"{device_or_module.file_path} has an invalid definition for {component_type} ({idx})."
                )
                return False
            ...
            if not isinstance(name, str):
                device_or_module.failureMessage = (
                    f"{device_or_module.file_path} has an invalid definition for {component_type} name ({idx})."
                )
                return False
            if eval_component[0] in known_names:
                device_or_module.failureMessage = (
                    f"{device_or_module.file_path} has duplicated names within {component_type} ({name})."
                )
                return False
        ...
        for position in position_set:
            if len(position_set[position]) > 1:
                ...
                if not validate_component_names(component_names):
                    device_or_module.failureMessage = (
                        f"{device_or_module.file_path} has duplicated positions within {component_type} ({position})."
                    )
                    return False
