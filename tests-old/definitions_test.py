...
from device_types import DeviceType, ModuleType, validate_components, verify_filename
...
def _get_diff_from_upstream():
    ...
    if "upstream" not in repo.remotes:
        repo.create_remote("upstream", NAUTOBOT_DT_LIBRARY_URL)
...
if USE_LOCAL_KNOWN_SLUGS:
    ...
    repo = Repo.clone_from(url=NAUTOBOT_DT_LIBRARY_URL, to_path=temp_dir.name)
...
def test_definitions(file_path, schema, change_type):
    ...
    assert verify_filename(this_device, (KNOWN_MODULES if not this_device.isDevice else None)), pytest.fail(
